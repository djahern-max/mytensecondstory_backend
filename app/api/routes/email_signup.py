...
from app.schemas.email_signup import (
    EmailSignupCreate,
...
async def create_email_signup(
    signup_data: EmailSignupCreate, db: Session = Depends(get_db)
):
    """Create a new email signup"""
    try:
        # Check if email already exists
        existing_signup = (
            db.query(EmailSignup).filter(EmailSignup.email == signup_data.email).first()
        )
        ...
        db_signup = EmailSignup(
            email=signup_data.email, full_name=signup_data.full_name
        )
...
async def get_signup_stats(db: Session = Depends(get_db)):
    ...
    try:
        ...
        recent_signups = (
            db.query(func.count(EmailSignup.id))
            .filter(EmailSignup.created_at >= yesterday)
            .scalar()
        )

        return EmailSignupStats(
            total_signups=total_signups or 0, recent_signups=recent_signups or 0
        )
