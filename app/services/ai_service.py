...
class AIEnhancementService:
    ...
    async def enhance_video_background(
        self, video_file_path: str, background_type: str, user_id: int
    ) -> Dict[str, Any]:
        """
    ...
    async def enhance_video_quality(
        ...
        try:
            result = await self.veo_client.enhance_existing_video(
                video_url=video_file_path, enhancement_type="quality", user_id=user_id
            )
    ...
    async def get_available_templates(self) -> Dict[str, Any]:
        ...
        return {"templates": VEO_PROMPT_TEMPLATES, "count": len(VEO_PROMPT_TEMPLATES)}
    ...
    async def get_user_job_history(
        ...
        user_jobs = [
            job for job in self.active_jobs.values() if job.get("user_id") == user_id
        ]
    ...
    async def generate_background_image(
        ...
        """
        Generate AI background image using OpenAI DALL-E 3
        Uses user's prompt directly without style interference
        """
        import openai
        import os

        logger.info(f"🎨 Generating DALL-E 3 background: {prompt}")

        # Initialize OpenAI client
        client = openai.AsyncOpenAI(api_key=os.getenv("OPENAI_API_KEY"))

        # Use user's prompt directly - let DALL-E 3 handle the creativity!
        full_prompt = prompt

        logger.info(f"🔍 DALL-E 3 prompt: {full_prompt}")
        ...
        logger.info(f"✅ DALL-E 3 background generated: {background_url}")
